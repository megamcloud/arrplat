...
from arrplat.resources.application.models import Application, OrgApplication, Message
...
from config import config, DEFAULT_PAGE, DEFAULT_PAGE_SIZE
...
class OrganizationListResource(Resource):
    ...
    def get(self, **kwargs):
        ...
        if not has_detail:
            ...
            org_staff_schema = OrgStaffSchema(only=("id", "organization.id", "organization.name", "org_department_staff", "job_title"))

            data = list()
            org_staff_list = db.session.query(Organization, OrgStaff).join(OrgStaff, Organization.id == OrgStaff.org_id).filter(
                OrgStaff.user_id == user.id).all()
...
class CreateOrganizationResource(Resource):
    @use_kwargs({
        ...
        "unique_id": fields.String(required=True, validate=lambda unique_id: True if 2 <= len(unique_id.strip()) <= 32 else False),
...
class OrgDepartmentResource(Resource):
    ...
    def put(self, org_id, org_department_id, **kwargs):
        ...
        try:
            user = get_current_user()
            org_department = db.session.query(OrgDepartment).join(OrgStaff,
                                                                  OrgStaff.org_id == OrgDepartment.org_id).filter(
                OrgStaff.user_id == user.id, OrgDepartment.id == org_department_id,
    ...
    def delete(self, org_id, org_department_id):
        ...
        if not org_department:
            return json_response(message="没有找不到福门", status=404)
...
class OrgAuthRoleStaffResource(Resource):
    ...
    def get(self, org_id, role_id, **kwargs):
       ...
       """
        ...
        staff_schema = OrgStaffSchema(many=True, only=('id', 'job_title', 'user', 'is_able', 'parent_superior', 'org_staff_role', 'org_department_staff'))
...
class OrgApplicationResource(Resource):
    ...
    def get(self, org_id, **kwargs):
        ...
        if is_all:
            org_obj = Application.query.order_by(Application.sort.asc()).all()
            org_schema = ApplicationSchema(many=True)
            res = org_schema.dump(org_obj).data
            ...
            # 查询出该组织下的所有app
            org_app_obj = Application.query.join(OrgApplication).filter(
                OrgApplication.organization_id == org_id).order_by(
                Application.sort.asc()).all()
            org_schema = ApplicationSchema(many=True)
