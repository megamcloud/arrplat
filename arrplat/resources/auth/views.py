...
from arrplat.resources.auth.models import OrgRoleConfig, OrgStaffRole, OrgAuthPowerConfig, OrgRolePower
from arrplat.resources.auth.schema import OrgRoleConfigSchema, OrgAuthPowerConfigSchema
from arrplat.resources.auth.sms_utils import send_phone_code
from arrplat.resources.organization.models import OrgStaff
from arrplat.resources.organization.schema import OrgStaffSchema
from arrplat.resources.user.services import UserSchema
from .services import add_access_token_to_redis
from extensions import ma, db
from arrplat.resources.user.models import User, UserInfo, UserPhoneCode
from arrplat.resources.core.logger import EnvLogger
...
logger = EnvLogger(__name__).env_getlogger()
...
class PhoneCodeResource(Resource):
    ...
    def post(self, **kwargs):
        ...
        if user:
            ...
            if user_phone_code and get_timestamp() - user_phone_code.update_time <= 300:
...
class PhoneLoginResource(Resource):
    ...
    def post(self, **kwargs):
        ...
        user = db.session.query(User).filter(
            User.phone == phone,
            UserPhoneCode.code == phone_code,
            # TODO
            # UserPhoneCode.update_time > current_timestamp - self.PHONE_VERIFY_CODE_EXPIRE
        ).first()
        if not user:
